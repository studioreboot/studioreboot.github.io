/*
    ...
    * destroy()                - deletion
    ...
    (9) animations
    ---------------
    MorphicAnims handle gradual transitions between one state and another over a
    ...
    MorphicAnims are generic and not limited to motion, i.e. they can also handle
    other transitions such as color changes, transparency fadings, growing,
    shrinking, turning etc.

    MorphicAnims need to be stepped by a scheduler, e. g. an interval function.
...
function isVowel (aCharacter) {
	return /[aeiouy]/ig.test(aCharacter);
}

function prefix (aString) {
    return ((isVowel(aString.substring(0, 1))) ? "an " : "a ") + aString;
}
...
class Point {
    ...
    normalize () {
        return new Point(isNaN(this.x) ? 0 : this.x, isNaN(this.y) ? 0 : this.y);
    }
...
class Rectangle {
    ...
    largerThan (aRect) {
        return this.origin.lt(aRect.origin) && this.corner.gt(aRect.corner);
    }

    smallerThan (aRect) {
        return this.origin.gt(aRect.origin) && this.corner.lt(aRect.corner);
    }
...
class BaseNode {
    ...
    toString () {
        return 'a BaseNode' + '[' + this.children.length.toString() + ']';
    }
    ...
    addChild (aBaseNode) {
        if (aBaseNode.parent)
            aBaseNode.parent.removeChild(aBaseNode);
        this.children.push(aBaseNode);
        aBaseNode.parent = this;
    }

    addChildFirst (aBaseNode) {
        if (aBaseNode.parent)
            aBaseNode.parent.removeChild(aBaseNode);
        this.children.unshift(aBaseNode);
        aBaseNode.parent = this;
    }

    removeChild (aBaseNode) {
        var idx = this.children.indexOf(aBaseNode);
        if (idx !== -1) {
    ...
    get allParents () {
        // includes myself
        var result = [this], selector = this;
        while (selector.parent) {
            result.push(selector.parent);
    ...
    dispatchEvent (name, ...args) {
        var selectors = this.allChildren;
        for (let i = 0; i < selectors.length; i++) {
            const sel = selectors[i];
            if (sel[name] && typeof sel[name] === "function") {
                sel[name].apply(sel, args);
            }
        }
    }
}
...
// Morph shared helpers:

function getNameOfFunction (aFunction) {
    var stringed = aFunction.toString()
    if ((!aFunction.name) && stringed.startsWith("class")) {
        var firstSpace = stringed.indexOf(" ");
        return stringed.substring(
            firstSpace, 
            stringed.indexOf(" ", firstSpace + 1)
        );
    } else if ((!aFunction.name) && stringed.startsWith("function")) {
        return (stringed.split(' ')[1].split('(')[0]);
    }
...
class Morph extends BaseNode {
    ...
    toString () {
		var className = getNameOfFunction(this.constructor);
        return (isVowel(className.substring(0, 1)) ? "an" : "a") + ' ' +
            className + ' ' + this.children.length.toString() + 
			' ' + this.bounds;
    }
    ...
    stepFrame () {
        ...
        leftover = this.fps > 0 ? (1000 / this.fps) - elapsed : 0;
        if (leftover < 1) {
            ...
            for (let i = 0; i < parents.length; i++) {
                ...
                if (parent != this) {
                    ...
                    for (let j = 0; j < children.length; j++) {
                        ...
                        if (child.bounds.containsRectangle(this.bounds) &&
                            child.bounds.largerThan(this.bounds) && !child.isShadow) {
                            return false;
                        }
                    }
                }
    ...
    visibleBounds () {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            if (visible && frame.bounds.intersects(visible)) {
                visible = frame.bounds.intersect(visible);
            } else {
                visible = null;
                break;
            }
    ...
    renderCachedTexture (ctx) {
        var bg = this.cachedTexture,
            cols = Math.ceil(this.width / bg.width), 
            lines = Math.ceil(this.height / bg.height), 
            x, y;
    ...
    drawing optimizations (un-finished.)
    ...
    // Morph drawing - testing visiblity:*/

    get isVisibleToUser () {
        return this.isVisible && this.visibleBounds() instanceof Rectangle;
    }

    isVisibleIn (aRect) {
        return aRect.intersects(this.bounds) || 
            aRect.containsRectangle(this.bounds);
    }

    // Morph drawing - children:

    get visibleChildren () {
        return this.children.filter(child => child.isVisibleToUser);
    }
    ...
    changed () {
		if (this.parent && !this.parent.bounds.intersects(this.bounds)) return;
        var w = this.root, bounds = this.visibleBounds();
        if (w instanceof WorldMorph && bounds instanceof Rectangle)
            w.broken.push(bounds.spread());
        if (this.parent)
    ...
    glideTo (endPoint, msecs, easing, onComplete) {
        var world = this.world, horizontal = new MorphicAnim(
            x => this.setLeft(x),
            () => this.left,
        ...
        world.animations.push(new MorphicAnim(
            y => this.setTop(y),
            () => this.top,
            -(this.top - endPoint.y),
            msecs === 0 ? 0 : msecs || 100,
    ...
    developersMenu () {
        ...
        var world = this.world instanceof Function ? this.world : this.world, userMenu = this.userMenu() ||
            (this.parent && this.parent.userMenu()), menu = new MenuMorph(this, getNameOfFunction(this.constructor));
        if (userMenu) {
    ...
    */
    ...
    escalateEvent (functionName, ...args) {
        ...
        if (handler[functionName]) {
            handler[functionName].apply(handler, args);
        }
    }
...
class HandleMorph extends Morph {
    ...
    mouseEnter (hand) {
        ...
        this.changed();
        if (hand) {
            hand.setCursorIcon("nwse-resize");
        }
    }
...
class SpeechBubbleMorph extends BoxMorph {
    ...
    */
    ...
    shadowImage (off, color) {
        // for "flat" design mode
        var fb, img, outline, sha, 
            ctx, offset = off || new Point(7, 7), 
            clr = color || new Color(0, 0, 0);
        fb = this.extent;
        img = this.getImage();
        outline = newCanvas(fb);
...
class DialMorph extends Morph {
    ...
    getValueOf (point) {
        var range = this.max - this.min, center = this.center, 
...
class SliderMorph extends CircleBoxMorph {
    ...
    fixLayout () {
        ...
        this.button.setPosition(
            new Point(posX, posY).normalize().add(this.position)
        );
    }
    ...
    mouseDownLeft (pos) {
        ...
        this.step = () => {
            ...
            if (world.hand.mouseButton) {
                ...
                this.button.setPosition(new Point(newX, newY).normalize());
                this.updateValue();
            } else {
                this.step = null;
            }
...
// InspectorMorph helpers:

function uniqueItemsOf (aArray) {
    return aArray.filter((item,
        index) => aArray.indexOf(item) === index);
}

function getKeysOf (aClass) {
    var proto = Object.getPrototypeOf(aClass);
    return uniqueItemsOf(
        Object.keys(aClass).concat(
            Object.keys(proto),
...
class InspectorMorph extends BoxMorph {
    ...
    updateCurrentSelection () {
        ...
        if (isNil(val)) {
            ...
            txt = val.toString().replaceAll("\t", "    ");
        }
        if (val instanceof Function && !txt.startsWith("class")) {
            var lines = txt.split("\n");
            if (lines[1] && lines[1].substring(0, 5) == "     ") {
                lines[0] = "function " + lines[0].substring(lines[0].indexOf("("), lines[0].length);
                for (let i = 1; i < lines.length; i++) {
                    lines[i] = lines[i].substring(4, lines[i].length);
                }
                lines.unshift("");
                lines.unshift("// declared in " + this.getDeclarationOf(sel));
                txt = lines.join("\n");
                if (!txt.endsWith("}")) {
                    txt += "}"
                }
    ...
    // InspectorMorph getting property declaration:

    getDeclarationOf (aProperty) {
        var ptr = this.target, found = false, proto;
        while (Object.getPrototypeOf(ptr) && !found) {
            proto = Object.getPrototypeOf(ptr);
            found = getKeysOf(ptr).includes(aProperty) || 
                Object.getOwnPropertyNames(Object.getPrototypeOf(ptr)).includes(aProperty) ||
                    Object.keys(ptr).includes(aProperty);
            ptr = proto;
        }
        return ptr.constructor.name;
    }

    // InspectorMorph getting target properties:

    getPropertiesOf (aTarget) {
        var names = Object.keys(aTarget).concat(Reflect.ownKeys(aTarget)), ptr = aTarget;
        while (Object.getPrototypeOf(ptr)) {
            names = names.concat(Object.getOwnPropertyNames(Object.getPrototypeOf(ptr)));
            ptr = Object.getPrototypeOf(ptr);
        }
        return uniqueItemsOf(names.filter(name => 
			!(name.substring(0, 1) == "_" || /callee|caller|arguments/ig.test(name))
		));
    }
    ...
    fixLayout () {
        ...
        if (!this.extent.eq(this.lastExtent) || !this.position.eq(this.lastPosition)) {
            this.removeShadow();
            this.addShadow();
        }
...
class MenuMorph extends BoxMorph {
    ...
    addItem(
        ...
        shortcut = null,
        verbatim = null // optional bool, don't translate if true
    ) {
        /*
    ...
    popup (world, pos) {
        ...
        this.addShadow(new Point(2.5, 2.5), 80);
        this.keepWithin(world);
...
class StringMorph extends Morph {
    ...
    render (ctx) {
        var start, 
            ...
            txt = this.isPassword ? this.password('*', this.text.length) : 
                this.text;
    ...
    developersMenu () {
        ...
        menu.addItem(
            this.isBold ? "normal weight" : "bold",
            "toggleWeight"
        );
        menu.addItem(
            this.isItalic ? "normal style" : "italic",
            "toggleItalic"
        );
        menu.addItem(
            this.isShowingBlanks ? "hide blanks" : "show blanks",
            "toggleShowBlanks"
        );
        menu.addItem(
            this.isPassword ? "show characters" : 
                "hide characters",
            "toggleIsPassword"
        )
        return menu;
    ...
    enableSelecting () {
        this.mouseDownLeft = function (pos) {
            var crs = this.root.cursor, already = crs ? crs.target === this : false;
            if (this.world.currentKey === 16) {
                this.shiftClick(pos);
            } else {
                this.clearSelection();
...
class TextMorph extends Morph {
    ...
    changed = StringMorph.prototype.changed;
    ...
    parse () {
        var paragraphs = this.text.split('\n'), 
            context = this.measureCtx, oldline = '', 
            newline, w, slot = 0;
    ...
    slotAt (aPoint) {
        ...
        // in account how far from the middle of the character it is,
        // so the cursor can be moved accordingly
        var charX, row = 0, col = 0, 
            columnLength, 
            shadowHeight = Math.abs(this.shadowOffset.y), 
            ctx = this.measureCtx, textWidth;
    ...
    execute (aString) {
        if (this.receiver.evaluateString) {
            return this.receiver.evaluateString(aString);
        }
        return Function.apply(null, ["stringToExecute", "return eval(stringToExecute)"]).call(this.receiver, aString);
    }

    doIt () {
        this.execute(this.lastSelection);
        this.edit();
    }

    showIt () {
        var result = this.execute(this.lastSelection);
        if (result !== null) {
            this.inform(result);
        }
    }
    ...
    inspectIt () {
        var result = this.execute(this.lastSelection), world = this.world, inspector;
        if (isObject(result)) {
	...
	fixSelf = StringMorph.prototype.fixSelf;
}

// BevelMorph //////////////////////////////////////////////////////////

// I look like a button when inverted.

class BevelMorph extends Morph {
    constructor (solidColor) {
        super();
        this.inset = 5;
        this.lightness = 30;
        this.darkness = 30;
        this.solidColor = !isNil(solidColor) ? solidColor : MorphicPreferences.isFlat;
        this.inverted = false;
    }
    ...
    renderBottomRight (ctx) {
        var w = this.width,
            h = this.height,
            dark = this.color.darker(this.darkness).toString(),
            darkc = this.color.darker(this.darkness),
            light = this.color.lighter(this.lightness).toString(),
            lightc = this.color.lighter(this.lightness),
            inset = this.inset,
        ...
        if (this.inverted) {
            ...
            grad.addColorStop(0, lightc.darker(5).toString());
            grad.addColorStop(1, color);

            ctx.fillStyle = this.solidColor ? lightc.darker(5).toString() : grad;
        } else {
            grad = ctx.createLinearGradient(w, 0, w - inset, 0);
            grad.addColorStop(0, dark);
            grad.addColorStop(1, color);

            ctx.fillStyle = this.solidColor ? dark : grad;
        }
        ...
        if (this.inverted) {
            ...
            ctx.fillStyle = this.solidColor ? light : grad;
        } else {
            grad = ctx.createLinearGradient(0, h, 0, h - inset);
            grad.addColorStop(0, darkc.darker(5).toString());
            grad.addColorStop(1, color);

            ctx.fillStyle = this.solidColor ? darkc.darker(5).toString() : grad;
        }
    ...
    renderTopLeft (ctx) {
        ...
        if (this.inverted) {
            grad = ctx.createLinearGradient(0, 0, 0, inset);
            grad.addColorStop(0, darkc.darker(5).toString());
            grad.addColorStop(1, color);
...
// TriggerMorph inherits from BevelMorph:

class TriggerMorph extends BevelMorph {
    ...
    constructor(
        ...
        action = null,
        labelString = " ",
        fontSize = MorphicPreferences.menuFontSize,
        fontStyle = "sans-serif",
        environment = null,
        hint = null,
        labelColor = BLACK,
        ...
        this.label = null;
        this.hint = hint; // null, String, or Function
    ...
    mouseEnter (hand) {
        ...
        if (hand) {
            hand.setCursorIcon(this.cursorIcon);
        }
    }
...
class MenuItemMorph extends TriggerMorph {
    ...
    popUpSubmenu () {
        var menu = this.parentThatIsA(MenuMorph), world = this.world, scroller;
...
// FrameMorph inherits from Morph:
    ...
    fullDrawOn (ctx, aRect) {
        ...
        this.children.forEach(child => {
            if (child instanceof ShadowMorph) {
                shadow = child;
            } else /*if (child.isVisibleToUser && child.isVisibleIn(clipped))*/ {
...
class ScrollFrameMorph extends FrameMorph {
    ...
    scrollX (steps) {
        var cl = this.contents.left, l = this.left, cw = this.contents.width, r = this.right, newX;
    ...
    scrollY (steps) {
        var ct = this.contents.top, t = this.top, ch = this.contents.height, b = this.bottom, newY;
    ...
    mouseDownLeft (pos) {
        ...
        var world = this.root, hand = world.hand, oldPos = pos, deltaX = 0, deltaY = 0, friction = 0.8;
    ...
    startAutoScrolling () {
        var inset = MorphicPreferences.scrollBarSize * 3, world = this.world, hand, inner, pos;
...
// I am a Morph that draws a canvas to the World
// I do NOT just hold a single image of a canvas.
// The canvas can change and so do I.

class CanvasMorph extends Morph {
    constructor (canvas) {
        super();
        this.canvas = canvas;
        if (!this.canvas) {
...
class BouncerMorph extends Morph {
    ...
    // BouncerMorph developers menu:

    developersMenu () {
        var menu = super.developersMenu();
        menu.addLine();
        menu.addItem(
            this.isStopped ? "start" : "stop",
            "toggleActive"
        );
        return menu;
    }
...
class HandMorph extends Morph {
    ...
    update () {
        var selector = this.morphAtPointer();
        if (selector.mouseOver) {
            selector.mouseOver(this);
        }
        this.children.forEach(child => {
            if (child.whileDragged) {
                child.whileDragged(this);
            }
        })
    }
    ...
    changed () {
        if (this.world !== null) {
            var b = this.cachedFullBounds || this.fullBounds();
            if (!b.extent.eq(ZERO)) {
    ...
    */
    ...
    setCursorIcon (icon) {
        if (MorphicPreferences.cursorReacts) {
            this.world.worldCanvas.style.cursor = icon;
        } else {
            this.world.worldCanvas.style.cursor = "default";
        }
    }
    ...
    */
    ...
    processMouseDown (event) {
        var morph, actualClick, posInDocument = getDocumentPositionOf(this.world.worldCanvas);
    ...
    processMouseUp () {
        var morph = this.morphAtPointer(), context, contextMenu, expectedClick;
        ...
        var files = event instanceof FileList ? event
            : event.target.files || event.dataTransfer.files, file, fileCount, url = event.dataTransfer ?
                event.dataTransfer.getData('URL') : null, txt = event.dataTransfer ?
                    event.dataTransfer.getData('Text/HTML') : null, suffix, 
            src, 
...
class WorldMorph extends FrameMorph {
    ...
    updateBroken () {
        ...
        if (this.doDrawBroken) this.drawBroken(ctx);
    ...
    contextMenu () {
        ...
        if (this.isDevMode) {
			...
			menu.addItem(
				this.doDrawBroken ? "hide broken" : "show broken",
				"toggleBroken",
				this.doDrawBroken ? "hides the rects that are currently \"dirty\"" :
					"shows the \"dirty\" parts of the World."
			)
            menu.addLine();
        }
        if (this.isDevMode) {
	...
	toggleBroken () {
		this.doDrawBroken = !this.doDrawBroken;
		this.broken.push(this.bounds.copy());
	}
